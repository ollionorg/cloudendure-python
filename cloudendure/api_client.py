...
class ApiClient(object):
    ...
    ##
    ## HERE: This was included to persist the auth setup for CE.  As of yet, I see no
    ## way to do it via the mechanism above (see the `auth in auth_settings` logic
    ## above)
    def set_auth_from_last_response(self):
        if not self.last_response:
            raise OpenApiException(
                "no last_response: There must have been a successful request to set auth."
            )

        cookie = self.last_response.getheader("Set-Cookie")

        # pull the values
        token = re.search('XSRF-TOKEN="(.+)"', cookie).group(1)
        session = re.search("(session=.*?);", cookie).group(1)

        ## set them properly
        self.set_session_auth(token, session)

    def set_session_auth(self, token, session):
        self.set_default_header("X-XSRF-TOKEN", token)
        self.cookie = session
...
class Endpoint(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        api_instance.accounts_account_id_get  # this is an instance of the class Endpoint
        api_instance.accounts_account_id_get()  # this invokes api_instance.accounts_account_id_get.__call__()
        which then invokes the callable functions stored in that endpoint at
        api_instance.accounts_account_id_get.callable or self.callable in this class
