        ...
        while True:
            try:
                ...
                if replicated_storage_bytes == total_storage_bytes:
                    ...
                    print 'Replicated ' + str(replicated_storage_bytes
                                              ) + ' out of ' + str(total_storage_bytes) + ' bytes'
...
def set_blueprint(args, machine_id, project_id):
    ...
    resp = session.patch(
        url=HOST + endpoint + 'projects/' + project_id + '/blueprints/' + blueprint['id'], data=json.dumps(blueprint)
    )
...
def launch_target_machine(args, machine_id, project_id):
    ...
    resp = session.post(
        url=HOST + endpoint + 'projects/' + project_id + '/launchMachines',
        data=json.dumps({
            'items': [items],
            'launchType': 'TEST'
        })
...
def login(args):
    ...
    resp = session.post(
        url=HOST + endpoint + 'login', data=json.dumps({
            'username': args.user,
